...
public class ADSMaterialsLighting : EditorWindow
{
    ...
    string bannerText;
    string helpURL;
    static ADSMaterialsLighting Window;

    [MenuItem("BOXOPHOBIC/Advanced Dynamic Shaders/Materials Lighting")]
    public static void ShowWindow()
    {

        Window = GetWindow<ADSMaterialsLighting>(false, "ADS Materials Lighting", true);
    ...
    void OnEnable()
    {
        ...
        bannerText = "ADS Materials Lighting";
    ...
    void DrawMessage()
    {
        ...
        EditorGUILayout.HelpBox("The ADS Materials Lighting tool will change all the selected materials in the project that are using ADS Shaders. When using the Get from Tag option, " +
    ...
    void DrawInspector()
    {
        ...
        GUILayout.BeginHorizontal();
        if (materialLighting == 0 || materialLighting == 4 || (Selection.objects.Length == 0 && selectionType == 1))
            GUI.enabled = false;
        else
            GUI.enabled = true;
    ...
    void ApplyShaderLightingTag()
    {
        ...
        for (int i = 0; i < allADSMaterials.Count; i++)
        {
            ...
            {
                if (materialLighting == 3 && (material.shader.name.Contains("Cloth") || material.shader.name.Contains("Generic") || material.shader.name.Contains("Legacy") || material.shader.name.Contains("Tree Bark")))
                    materialRenamed = materialName.Replace("Simple Lit", ShaderLightingEnumStandard[materialLighting - 1]);
                else
            ...
            {
                if (materialLighting == 3 && (material.shader.name.Contains("Cloth") || material.shader.name.Contains("Generic") || material.shader.name.Contains("Legacy") || material.shader.name.Contains("Tree Bark")))
                    materialRenamed = materialName.Replace("Standard Lit", ShaderLightingEnumStandard[materialLighting - 1]);
                else
                    materialRenamed = materialName.Replace("Standard Lit", ShaderLightingEnumStandard[materialLighting]);
            }
            else if (material.name.Contains("Advanced Lit"))
            {
                if (materialLighting == 3 && (material.shader.name.Contains("Cloth") || material.shader.name.Contains("Generic") || material.shader.name.Contains("Legacy") || material.shader.name.Contains("Tree Bark")))
                    materialRenamed = materialName.Replace("Advanced Lit", ShaderLightingEnumStandard[materialLighting - 1]);
                else
                    materialRenamed = materialName.Replace("Advanced Lit", ShaderLightingEnumStandard[materialLighting]);
            }
            else
            {
                if (materialLighting == 3 && (material.shader.name.Contains("Cloth") || material.shader.name.Contains("Generic") || material.shader.name.Contains("Legacy") || material.shader.name.Contains("Tree Bark")))
                    materialRenamed = materialName + " (" + ShaderLightingEnumStandard[materialLighting - 1] + ")";
                else
        ...
        for (int i = 0; i < allADSMaterials.Count; i++)
        {
            ...
            if (materialLighting == 4)
            {
                if (material.name.Contains("Simple Lit"))
                {
                    ...
                    else if (material.shader.name.Contains("Grass") && material.shader.name.Contains("Legacy") == false)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/Simple Lit/Grass");
                    else if (material.shader.name.Contains("Grass") && material.shader.name.Contains("Legacy"))
                ...
                {
                    if (material.shader.name.Contains("Grass") && material.shader.name.Contains("Legacy") == false)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/Advanced Lit/Grass");
                    else if (material.shader.name.Contains("Plant"))
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/Advanced Lit/Plant");
                    else if (material.shader.name.Contains("Tree Leaf"))
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/Advanced Lit/Tree Leaf");
                }
            }
            else
            {
                if (material.shader.name.Contains("Cloth"))
                {
                    if (materialLighting == 3)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/" + ShaderLightingEnumStandard[materialLighting - 1] + "/Cloth");
                    else
                ...
                {
                    if (materialLighting == 3)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/" + ShaderLightingEnumStandard[materialLighting - 1] + "/Generic");
                    else
                ...
                {
                    if (materialLighting == 3)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/" + ShaderLightingEnumStandard[materialLighting - 1] + "/Grass (Legacy)");
                    else
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/" + ShaderLightingEnumStandard[materialLighting] + "/Grass (Legacy)");
                }
                else if (material.shader.name.Contains("Plant"))
                {
                ...
                {
                    if (materialLighting == 3)
                        material.shader = Shader.Find("BOXOPHOBIC/Advanced Dynamic Shaders/" + ShaderLightingEnumStandard[materialLighting - 1] + "/Tree Bark");
                    else
