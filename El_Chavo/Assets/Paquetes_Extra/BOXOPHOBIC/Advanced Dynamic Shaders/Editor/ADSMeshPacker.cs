...
public class ADSMeshPacker : EditorWindow
{
    ...
    public bool motionVariationInvert;
    public bool vertexAOInvert;
    ...
    void DrawPacking()
    {
        ...
        if (meshPacking == MeshPackingEnum.Generic)
        {
            ...
            if (motionVariation == 0)
            {
                motionVariationInvert = false;
            }
            else
            {
                motionVariationInvert = EditorGUILayout.Toggle("Variation Invert", motionVariationInvert);
            }
        }
        ...
        else if (meshPacking == MeshPackingEnum.Tree)
        {
            if (treeType == TreeTypeEnum.Custom)
            {
                ...
                {
                    vertexAOInvert = EditorGUILayout.Toggle("Vertex AO Invert", vertexAOInvert);
                }
    ...
    float[] GetMeshChannel(int c, float val)
    {
    ...
    void GenericPacking()
    {
        ...
        var mMask = GetMeshChannel(motionMask, 0.0f);
        var mVariation = GetMeshChannel(motionVariation, 2.0f);

        for (int i = 0; i < inputMesh.vertexCount; i++)
        {
            colors[i] = new Color(SourceInvert(mMask[i], motionMaskInvert), 0.0f, 0.0f, SourceInvert(mVariation[i], motionVariationInvert));
        }
    ...
    void PlantPackingCustom()
    {
        ...
        var mMask = GetMeshChannel(motionMask, 0.0f);
        var mMask2 = GetMeshChannel(motionMask2, 0.0f);
        var mVariation = GetMeshChannel(motionVariation, 2.0f);

        for (int i = 0; i < inputMesh.vertexCount; i++)
        {
            colors[i] = new Color(SourceInvert(mMask[i], motionMaskInvert), 0.0f, SourceInvert(mMask2[i], motionMask2Invert), SourceInvert(mVariation[i], motionVariationInvert));
            UV4[i] = new Vector3(inputMesh.vertices[i].x, inputMesh.vertices[i].y, inputMesh.vertices[i].z);
        }
    ...
    void TreePackingCustom()
    {
        ...
        var mMask = GetMeshChannel(motionMask, 0.0f);
        var mMask2 = GetMeshChannel(motionMask2, 0.0f);
        var mMask3 = GetMeshChannel(motionMask3, 0.0f);

        var mVariation = GetMeshChannel(motionVariation, 2.0f);
        var vAO = GetMeshChannel(vertexAO, 1.0f);

        for (int i = 0; i < inputMesh.vertexCount; i++)
        {
            colors[i] = new Color(SourceInvert(mMask[i], motionMaskInvert), SourceInvert(mMask2[i], motionMask2Invert), SourceInvert(mMask3[i], motionMask3Invert), SourceInvert(mVariation[i], motionVariationInvert));
            UV0[i] = new Vector3(UV0[i].x, UV0[i].y, SourceInvert(vAO[i], vertexAOInvert));
        }
    ...
    void TreePackingSpeedTree7()
    {
        ...
        for (int i = 0; i < inputMesh.vertexCount; i++)
        {
            ...
            UV0[i] = new Vector3(UV0[i].x, UV0[i].y, inputMesh.colors[i].r);
        }
    ...
    void TreePackingTreeIt()
    {
        ...
        for (int i = 0; i < inputMesh.vertexCount; i++)
        {
            ...
            UV0[i] = new Vector4(UV0[i].x, UV0[i].y, 1.0f);
        }
